...
function jieqi_topic_vars($topic, $enableubb = true)
{
    ...
    $posterid = $topic->getVar("posterid");
	if($posterid > 0)
	{
	 $rets = mysql_query("SELECT * FROM jieqi_system_users WHERE  uid= $posterid");
	 $plevel = mysql_fetch_object($rets);
	    jieqi_getconfigs('system', 'honors');
        jieqi_getconfigs('system', 'vips');
		$ret["honorid"]=jieqi_gethonorid($plevel->score, $jieqiHonors);
		$ret["honor"] = $jieqiHonors[$ret["honorid"]]['name'][intval($plevel->workid)];
		$ret["vipid"]=jieqi_getvipid($plevel->expenses, $jieqiVips);
        $ret["vip"] = $jieqiVips[$ret["vipid"]]['caption'];
	}
...
function jieqi_post_vars($post, $configs = array(), $addvars = array(), $enableubb = true)
{
	...
	if (0 < $ret["userid"]) {
		...
		$vipid=jieqi_getvipid($post->getVar("expenses"), $jieqiVips);
		$ret["vipid"] = $vipid;
