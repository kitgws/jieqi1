...
function jieqi_ajax(url) {
	...
	this.submitForm = function(form) {
		if(this.requestFile == null) this.requestFile = $_(form).action;
...
function ajax_gethtml(){
	...
	if(tip != ""){
		$_(tip).innerHTML = "";
		$_(tip).style.display = "";
		if(loading != "") ajax.onLoading = function(){$_(tip).innerHTML = loading;};
		if(loaded != "") ajax.onLoaded = function(){$_(tip).innerHTML = loaded;};
		if(interactive != "") ajax.onInteractive = function(){$_(tip).innerHTML = interactive;};
		ajax.onError = function() {$_(tip).innerHTML = "ERROR:"+ajax.responseStatus[1]+"("+ajax.responseStatus[0]+")";};
		ajax.onFail = function() {$_(tip).innerHTML = "Your browser does not support AJAX!";};
	}
	ajax.onCompletion = function(){
		if(tip != ""){
			if(completed != "") $_(tip).innerHTML = completed;
			else if(tip != show){
				$_(tip).innerHTML = "";
				$_(tip).style.display = "none";
			}
		}
		$_(show).innerHTML = ajax.response;
	};
	ajax.runAJAX();
}
